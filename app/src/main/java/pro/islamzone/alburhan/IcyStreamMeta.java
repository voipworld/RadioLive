...
public class IcyStreamMeta
{
    ...
    synchronized private void retreiveMetadata() throws IOException
    {
        ...
        while (metaDataOffset > 0) {
            metaDataOffset -= stream.skip(metaDataOffset);
        }
        int b = stream.read();
        byte[] bytes = new byte[b * 16];
        stream.read(bytes);

        // Set the data
        metadata = IcyStreamMeta.parseMetadata(new String(bytes, Charset.forName("utf-8")));
    ...
    public static Map<String, String> parseMetadata(String metaString)
    {
        ...
        for (String metaPart : metaParts) {
            if (metaPart.contains("='") && metaPart.endsWith("'")) {
                int endIndex = metaPart.indexOf("='");
                metadata.put(metaPart.substring(0, endIndex), metaPart.substring(endIndex + 2, metaPart.length() - 1));
            }
        }
